...
var import_elliptic4 = require("elliptic");
...
var KeyType = /* @__PURE__ */ ((KeyType3) => {
  KeyType3[KeyType3["k1"] = 0] = "k1";
  KeyType3[KeyType3["r1"] = 1] = "r1";
  KeyType3[KeyType3["wa"] = 2] = "wa";
  KeyType3[KeyType3["rsa"] = 100] = "rsa";
  KeyType3[KeyType3["eth"] = 101] = "eth";
  return KeyType3;
})(KeyType || {});
...
var privateKeyToLegacyString = (key) => {
  if (key.type === 0 /* k1 */ && key.data.length === privateKeyDataSize) {
    ...
    const digest = new Uint8Array(
      (0, import_hash.sha256)().update(
        (0, import_hash.sha256)().update(whole).digest()
      ).digest()
    );
...
var symbolToString = ({ name, precision }) => {
...
var createAbiTypes = () => {
  ...
  initialTypes.set("secondary_indices", createType({
    ...
    fields: [
      { name: "name", typeName: "name", type: void 0 },
      { name: "secondary_index_def", typeName: "secondary_index_def", type: void 0 }
    ],
  ...
  initialTypes.set("kv_table_entry_def", createType({
    ...
    fields: [
      { name: "type", typeName: "string", type: void 0 },
      { name: "primary_index", typeName: "primary_key_index_def", type: void 0 },
      { name: "secondary_indices", typeName: "secondary_indices", type: void 0 }
    ],
    serialize: serializeStruct,
    deserialize: deserializeStruct
  }));
  initialTypes.set("kv_table", createType({
    ...
    fields: [
      { name: "name", typeName: "name", type: void 0 },
      { name: "kv_table_entry_def", typeName: "kv_table_entry_def", type: void 0 }
    ],
    serialize: serializeObject,
    deserialize: deserializeObject
  }));
  initialTypes.set("abi_def", createType({
    ...
    fields: [
      ...
      { name: "ricardian_clauses", typeName: "clause_pair[]", type: void 0 },
      { name: "error_messages", typeName: "error_message[]", type: void 0 },
      { name: "abi_extensions", typeName: "extensions_entry[]", type: void 0 },
      { name: "variants", typeName: "variant_def[]$", type: void 0 },
      { name: "action_results", typeName: "action_result[]$", type: void 0 },
...
var createTransactionTypes = () => {
  ...
  initialTypes.set("action", createType({
    ...
    fields: [
      ...
      { name: "name", typeName: "name", type: void 0 },
      { name: "authorization", typeName: "permission_level[]", type: void 0 },
  ...
  initialTypes.set("transaction", createType({
    ...
    fields: [
      ...
      { name: "transaction_extensions", typeName: "extension", type: void 0 }
    ],
    serialize: serializeStruct,
    deserialize: deserializeStruct
  }));
  ...
  if (abi && abi.structs) {
    for (const { name, base, fields } of abi.structs) {
      types.set(name, createType({
        name,
        baseName: base,
        fields: fields.map(({ name: n, type }) => ({ name: n, typeName: type, type: void 0 })),
        serialize: serializeStruct,
        deserialize: deserializeStruct
      }));
    }
...
var serializeAnyvar = (buffer, anyvar) => {
  ...
  if (anyvar === null) {
    ...
    [def, value] = [anyvarDefs[anyvar.type], anyvar.value];
  } else {
...
var deserializeAnyvarShort = (buffer) => {
  return deserializeAnyvar(buffer, new SerializerState({ useShortForm: true }));
};
...
var import_elliptic3 = require("elliptic");
...
// src/cryptography/conversions.ts
...
var constructElliptic = (type) => {
  ...
  return new import_elliptic.ec("p256");
};
var sha2563 = (data) => {
...
var PublicKey = class {
  ...
  static fromString(publicKeyStr, ec4) {
    const key = stringToPublicKey(publicKeyStr);
    if (!ec4) {
      ec4 = constructElliptic(key.type);
    }
    return new PublicKey(key, ec4);
  }
  static fromElliptic(publicKey, keyType, ec4) {
    const x = publicKey.getPublic().getX().toArray("be", 32);
    const y = publicKey.getPublic().getY().toArray("be", 32);
    if (!ec4) {
      ec4 = constructElliptic(keyType);
    }
...
var Signature = class {
  constructor(signature, ec4) {
    this.signature = signature;
    this.ec = ec4;
  }
  static fromString(sig, ec4) {
    const signature = stringToSignature(sig);
    if (!ec4) {
      ec4 = constructElliptic(signature.type);
    }
    return new Signature(signature, ec4);
  }
  static fromElliptic(ellipticSig, keyType, ec4) {
  ...
  toElliptic() {
    ...
    const s = new import_bn.default(this.signature.data.slice(lengthOfR + 1, lengthOfR + lengthOfS + 1));
...
var PrivateKey = class {
  ...
  static fromElliptic(privKey, keyType, ec4) {
    if (!ec4) {
      ec4 = constructElliptic(keyType);
    }
  ...
  static fromString(keyString, ec4) {
    const privateKey = stringToPrivateKey(keyString);
    if (!ec4) {
      ec4 = constructElliptic(privateKey.type);
    }
    return new PrivateKey(privateKey, ec4);
  }
  toElliptic() {
...
// src/cryptography/key.ts
var import_elliptic2 = require("elliptic");
var generateKeyPair = (type, options = {}) => {
  ...
  let ec4;
  if (type === 0 /* k1 */) {
    ec4 = new import_elliptic2.ec("secp256k1");
  } else {
    ec4 = new import_elliptic2.ec("p256");
  }
  const ellipticKeyPair = ec4.genKeyPair(options.ecOptions);
  const publicKey = PublicKey.fromElliptic(ellipticKeyPair, type, ec4);
  const privateKey = PrivateKey.fromElliptic(ellipticKeyPair, type, ec4);
  return { publicKey, privateKey };
};

// src/cryptography/jsSignatureProvider.ts
var defaultEc = new import_elliptic3.ec("secp256k1");
var digestFromSerializedData = (chainId, serializedTransaction, serializedContextFreeData, e = defaultEc) => {
...
var Api = class {
  ...
  async transact(transaction, {
    ...
    if (!transactionHeader2) {
      if (typeof blocksBehind === "number" && useLastIrreversible) {
        ...
        transactionHeader2 = await this.generateTapos(info, transaction, blocksBehind, useLastIrreversible, expireSeconds);
        ;
      }
  ...
  async generateTapos(info, transaction, blocksBehind = 3, useLastIrreversible, expireSeconds) {
    ...
    return { ...transactionHeader(refBlock, expireSeconds), ...transaction };
  }
  ...
  async getAccountKeys(actor, permission) {
    try {
      ...
      if (relevantPermission) {
        return relevantPermission.required_auth.keys.map(({ key }) => PublicKey.fromString(key).toString());
      }
    } catch (e) {
      console.log(e);
    }
  ...
  async matchRsaKeyToSignature({ actor, transaction }) {
    const { signature, nonce, trx } = transaction.actions[0].data;
    ...
    for (const cred of creds) {
      ...
      const signedDigest = new Uint8Array(import_base64url.default.toBuffer(clientJson.challenge).buffer);
  ...
  async matchEthKeyToSignature({ actor, transaction }) {
    const { signature, nonce, trx } = transaction.actions[0].data;
    let creds = await this.rpc.fetchCredentials(actor);
    ...
    for (const cred of creds) {
      ...
      const actualDigest = arrayToHex(actualDigestArray);
  ...
  async matchWaKeyToSignature({ signature, key, digest }) {
    try {
      const fullSignature = stringToSignature(signature);
      const sigData = new SerialBuffer({ array: fullSignature.data });
      const r1Signature = new Signature({
        type: 1 /* r1 */,
        data: sigData.getUint8Array(65)
      }, new import_elliptic4.ec("p256"));
      ...
      const actualR1KeyBuffer = new SerialBuffer({
        array: waPublicKey.key.data.slice(0, 33)
      });
      const actualR1Key = publicKeyToString({
        type: 1 /* r1 */,
  ...
  async checkIfKeysMatchTransaction({
    ...
    if (transaction.actions.length === 1 && transaction.actions[0].account === "webauthn" && transaction.actions[0].name === "exec" && permission === "active") {
      ...
      transaction.context_free_actions = await this.serializeActions(transaction.context_free_actions || []), transaction.actions = await this.serializeActions(transaction.actions);
      const serializedTransaction = this.serializeTransaction(transaction);
      const serializedContextFreeData = this.serializeContextFreeData(transaction.context_free_data);
      ...
      const verified = keys.some((key) => {
        const publicKey = PublicKey.fromString(key);
        if (signatureType === 2 /* wa */) {
          return this.matchWaKeyToSignature({
            signature: signature.toString(),
            key: publicKey.toString(),
            digest
          });
        } else if ([0 /* k1 */, 1 /* r1 */].includes(signatureType)) {
...
async function getValidKycProviders() {
  try {
    ...
    this.validKycProviders = rows.reduce((acc, provider) => {
      if (!provider.blisted) {
...
async function isLightKYCVerified(account) {
  ...
  const resultsWithKycStatus = users.map((user) => {
    ...
    user.isTier1KYCVerified = tier1Kyc.every((lightKycField) => levelsResult.includes(lightKycField));
...
var JsonRpc = class {
  constructor(endpoints = [], options = {}) {
    ...
    this.initialEndpoints = endpoints.map((endpoint) => endpoint.replace(/\/$/, ""));
  ...
  async get_block_info(blockNum) {
    return await this.fetch("/v1/chain/get_block_info", { block_num: blockNum }, 0, false);
  }
  ...
  async push_transaction({ signatures, compression = 0, serializedTransaction, serializedContextFreeData }) {
    try {
      return await this.fetch("/v1/chain/push_transaction", {
        signatures,
        compression,
        packed_context_free_data: arrayToHex2(serializedContextFreeData || new Uint8Array(0)),
        packed_trx: arrayToHex2(serializedTransaction)
      });
    } catch (e) {
  ...
  async push_ro_transaction({ signatures, compression = 0, serializedTransaction }, returnFailureTraces = false) {
    return await this.fetch("/v1/chain/push_ro_transaction", {
      transaction: {
        signatures,
        compression,
        packed_context_free_data: arrayToHex2(new Uint8Array(0)),
        packed_trx: arrayToHex2(serializedTransaction)
      },
...
// src/cryptography/webauthn.ts
var import_elliptic5 = require("elliptic");
var WebAuthnSignatureProvider = class {
  constructor() {
    this.keys = /* @__PURE__ */ new Map();
  }
  async getAvailableKeys() {
  ...
  async sign({ chainId, requiredKeys, serializedTransaction, serializedContextFreeData }) {
    ...
    const signatures = [];
    for (const key of requiredKeys) {
      const id = hexToUint8Array(this.keys.get(key));
      ...
      const e = new import_elliptic5.ec("p256");
      const pubKey = e.keyFromPublic(stringToPublicKey(key).data.subarray(0, 33)).getPublic();
      const fixup = (x) => {
      ...
      const der = new SerialBuffer({ array: new Uint8Array(assertion.response.signature) });
      if (der.get() !== 48) {
      ...
      const recid = e.getKeyRecoveryParam(hash2, new Uint8Array(assertion.response.signature), pubKey);
