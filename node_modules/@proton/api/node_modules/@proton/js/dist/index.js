...
var decodeWaSignature = (key) => {
  ...
  const digest = new Uint8Array(sha256Array(whatItReallySigned.asUint8Array().slice()));
  ...
  const clientDataJSON = JSON.parse(Buffer.from(clientDataJSONBytes).toString("utf8"));
...
var JsSignatureProvider = class {
  ...
  async sign({ chainId, requiredKeys, serializedTransaction, serializedContextFreeData }) {
    const digest = digestFromSerializedData(chainId, serializedTransaction, serializedContextFreeData, k1Ec);
    const signatures = [];
...
var Api = class {
  ...
  serializeTransactionExtensions(transaction) {
    ...
    if (transaction.resource_payer) {
      ...
      transaction_extensions = [...transaction_extensions, [1, arrayToHex2(extensionBuffer.asUint8Array())]];
    }
  ...
  async matchRsaKeyToSignature({ action, credentials }) {
    ...
    for (const cred of credentials) {
      ...
      const signedDigest = new Uint8Array(import_base64url2.default.toBuffer(clientJson.challenge));
  ...
  async matchEthKeyToSignature({ action, credentials }) {
    const { signature, nonce, trx } = action.data;
    for (const cred of credentials) {
  ...
  async checkIfKeysMatchWaAction({
    ...
    if (!action.authorization.every((_) => _.actor === actor && _.permission === permission)) {
      console.log("WA RSA/ETH Authorization mismatch");
      return false;
    }
    const { account, key } = action.data;
    if (account !== actor) {
    ...
    try {
      if (transaction.actions.length === 1 && transaction.actions[0].account === "webauthn" && transaction.actions[0].name === "exec") {
        ...
        const verified = signatures.every((stringSignature) => {
          ...
          const digest = digestFromSerializedData(chainId, serializedTransaction, serializedContextFreeData, signature.getEc());
          return keys.some((key) => signature.verify(digest, PublicKey.fromString(key), false));
        });
        return verified;
      }
    } catch (e) {
